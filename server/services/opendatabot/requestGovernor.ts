...
export class OpendatabotRequestGovernor {
  private static instance: OpendatabotRequestGovernor;
  private queue: QueueItem[] = [];
  private activeCount = 0;
  private maxConcurrency = config.OPENDATABOT_MAX_CONCURRENT_REQUESTS;
  ...
  public static getInstance(): OpendatabotRequestGovernor {
    if (!OpendatabotRequestGovernor.instance) {
      OpendatabotRequestGovernor.instance = new OpendatabotRequestGovernor();
    }
    return OpendatabotRequestGovernor.instance;
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    if (!rateCheck.allowed) {
      ...
      console.warn(`[OpendatabotRequestGovernor] Rate limited. Postponing queue processing by ${rateCheck.waitMs}ms`);
...
export const governor = OpendatabotRequestGovernor.getInstance();
