...
import { CkanPackage, CkanSearchResponse, DataSourceResult } from "../types";
...
const parse = (value: unknown): CkanSearchResponse => {
  ...
  if (payload.success !== true || typeof result !== "object" || result === null) throw new Error("CKAN payload is invalid");
  const resultObject = result as Record<string, unknown>;
  if (typeof resultObject.count !== "number" || !Array.isArray(resultObject.results)) throw new Error("CKAN result is invalid");
  const packages = resultObject.results.map((item): CkanPackage => {
    ...
    if (typeof packageObject.id !== "string" || typeof packageObject.title !== "string" || typeof packageObject.name !== "string") {
    ...
    return {
      ...
      metadata_modified: typeof packageObject.metadata_modified === "string" ? packageObject.metadata_modified : undefined,
      organization: typeof packageObject.organization === "object" && packageObject.organization !== null
        ? {
            name: typeof (packageObject.organization as Record<string, unknown>).name === "string"
              ? (packageObject.organization as Record<string, unknown>).name as string
              : undefined,
            title: typeof (packageObject.organization as Record<string, unknown>).title === "string"
              ? (packageObject.organization as Record<string, unknown>).title as string
              : undefined,
          }
        : undefined,
      resources: Array.isArray(packageObject.resources)
        ? packageObject.resources.filter((resource): resource is Record<string, unknown> => typeof resource === "object" && resource !== null)
          .map((resource) => {
              ...
              datastore_active: typeof resource.datastore_active === "boolean" ? resource.datastore_active : undefined,
            };
            })
        : [],
    };
  ...
  return {
    ...
    result: {
      ...
      organization_list: Array.isArray(resultObject.organization_list)
        ? resultObject.organization_list.filter((item): item is { name: string; count: number } =>
...
export const search = async (query: string, rows: number): Promise<DataSourceResult<CkanSearchResponse>> => {
  ...
  return cachedFetch(cache, `${query}:${rows}`, "ckan", "data.gov.ua CKAN", sourceUrl, parse, "data.gov.ua open data license");
};
