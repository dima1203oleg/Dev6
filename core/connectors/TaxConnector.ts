...
export class TaxConnector extends BaseConnector {
  constructor() {
    super(
      'TaxConnector', 
      'ДПС Connector',
      'https://tax.gov.ua/api', // Placeholder - needs official API URL
      'v1.0',
      'API_KEY'
    );
  }
  ...
  async search(query: string): Promise<RawEvidence[]> {
    ...
    // TODO: Implement actual API call to DPS API
    return [];
  }

  async health_check(): Promise<ConnectorStatus> {
    // TODO: Implement real health check against DPS API
  ...
  get_production_validation(): ProductionValidation {
    return {
      ...
      documentation_url: 'https://tax.gov.ua/api',
