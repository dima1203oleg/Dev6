          ...
          <div className="p-6 space-y-8">
            ...
            <section className="space-y-4">
              ...
              <div className="bg-slate-950 rounded-xl border border-slate-800 overflow-hidden divide-y divide-slate-800">
                {[
                  { label: "Retrieved At", value: new Date(evidence.retrievedAt).toLocaleString(), icon: Clock },
                  { label: "Content Hash", value: evidence.contentHash || "sha256:0x492a...", icon: Code },
                  { label: "Confidence", value: `${(evidence.confidence * 100).toFixed(1)}%`, icon: Activity },
                ].map((item) => (
                  <div key={item.label} className="p-4 flex items-center justify-between group">
                    ...
                    <span className="text-xs font-mono text-slate-400 select-all">{item.value}</span>
                  </div>
                ))}
              </div>
            </section>
