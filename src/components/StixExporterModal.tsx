...
export default function StixExporterModal({ entity, onClose }: StixExporterModalProps) {
  ...
  const handleDownload = () => {
    if (activeFormat === "stix") {
      downloadFile(
        stixJsonString,
        `NEXUS_STIX21_${entity.code}_${entity.name.replace(/[^a-zA-Z0-9]/g, "_")}.json`,
        "application/json",
      );
      ...
      downloadFile(
        graphmlXmlString,
        `NEXUS_GRAPHML_${entity.code}_${entity.name.replace(/[^a-zA-Z0-9]/g, "_")}.graphml`,
        "application/xml",
      );
          ...
          <div className="flex items-center gap-2 bg-slate-900 p-1 rounded-xl border border-slate-800">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold uppercase transition-all cursor-pointer flex items-center gap-1.5 ${
                activeFormat === "stix" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-slate-200"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold uppercase transition-all cursor-pointer flex items-center gap-1.5 ${
                activeFormat === "graphml" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-slate-200"
              }`}
            >
