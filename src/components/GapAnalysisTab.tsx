...
export default function GapAnalysisTab() {
  ...
  const getComplexityColor = (val: number) => {
    switch (val) {
      case 1:
        return "text-emerald-400 bg-emerald-500/10 border-slate-800";
      case 2:
        return "text-blue-400 bg-blue-500/10 border-slate-800";
      case 3:
        return "text-amber-400 bg-amber-500/10 border-slate-800";
      default:
  ...
  const filteredGapItems =
    selectedCategory === "all" ? GAP_ITEMS : GAP_ITEMS.filter((item) => item.category === selectedCategory);
    ...
    >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-2 backdrop-blur-md">
        ...
        <p className="text-slate-300 text-xs leading-relaxed">
          Аналіз відповідності вимогам open-source продуктів та розрахунок ресурсів на створення внутрішньої унікальної
          інтелектуальної власності NEXUS Analytics. Визначте, які компоненти вимагають адаптації, заміни чи розробки з
          нуля.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-lg p-2 space-y-3" id="gap-filters">
            ...
            <div className="flex-1 p-2 sm:p-3 md:p-4 flex flex-col gap-3 sm:gap-4 bg-slate-950 h-full w-full">
              <button
                ...
                className={`w-full text-left px-2 py-1.5 rounded-lg border transition-all flex items-center justify-between ${selectedCategory === "all" ? "bg-blue-500/10 border-slate-800 text-indigo-300 font-semibold" : "bg-black/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border-slate-800 text-slate-300 hover:text-slate-200"}`}
              >
                <span>Усі категорії ({GAP_ITEMS.length})</span>
                <span className="text-xs bg-black/40 backdrop-blur-md px-2 py-1 rounded border border-slate-800">
                  Все
                </span>
              </button>
              {GAP_ITEMS.map((item) => (
                <button
                  ...
                  className={`w-full text-left px-2 py-1.5 rounded-lg border transition-all flex items-center justify-between ${selectedCategory === item.category ? "bg-blue-500/10 border-slate-800 text-indigo-300 font-semibold" : "bg-black/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border-slate-800 text-slate-300 hover:text-slate-200"}`}
                >
                  <span className="truncate">{item.categoryLabel}</span>
                  <span className="text-xs bg-black/40 backdrop-blur-md px-2 py-1 rounded border border-slate-800 uppercase font-mono">
                    {item.difficulty}
                  </span>
                </button>
              ))}
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-lg p-2 space-y-5" id="dynamic-estimator">
            ...
            <p className="text-xs text-slate-300 leading-normal">
              Оцінка ресурсів для створення <strong>Власної експертизи (з нуля)</strong>: Конекторів до реєстрів
              України, Entity Resolution Engine та онтології даних.
            </p>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span
                    className={`px-2 py-1 rounded font-medium text-xs border ${getComplexityColor(complexity.connectors)}`}
                  >
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span
                    className={`px-2 py-1 rounded font-medium text-xs border ${getComplexityColor(complexity.entityResolution)}`}
                  >
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span
                    className={`px-2 py-1 rounded font-medium text-xs border ${getComplexityColor(complexity.ontology)}`}
                  >
            ...
            <div
              className="bg-black/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] rounded-lg p-2 border border-slate-800 space-y-3"
              id="estimator-results"
            >
              ...
              <div className="bg-emerald-500/5 border border-slate-800 p-2 rounded-lg text-center">
                <DollarSign className="w-4 h-4 text-emerald-400 mx-auto mb-1" />
                <span className="text-xs text-slate-500 uppercase block">Бюджет розробки</span>
                <span className="text-xs font-mono font-bold text-emerald-400">${estimates.cost.toLocaleString()}</span>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-4" id="gap-cards-container">
            {filteredGapItems.map((item) => (
              <div
                key={item.id}
                className="bg-black/30 border border-slate-800/80 rounded-lg p-2 space-y-4 hover:border-slate-800/60 transition-colors"
              >
                {/* Card Title Header */}
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-black/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] rounded-lg border border-slate-800">
                  ...
                  <div className="text-right">
                    <span
                      className={`text-xs font-bold border px-2.5 py-0.5 rounded-full font-mono block ${getDifficultyColor(item.difficulty)}`}
                    >
                      {item.difficulty}
                    </span>
                ...
                {/* Card Description */}
                <p className="text-xs text-slate-300 leading-relaxed bg-black/40 p-2 rounded-lg border border-slate-800/60">
                  {item.description}
                </p>
