...
export default function D3HistoricalRiskTrendsWidget({
    ...
    () => [
      {
        ...
        description:
          "ДКІБ СБУ ініціювало розслідування щодо постачання електроніки подвійного призначення компанією СпецТехПостач.",
      ...
      },
      {
        ...
        description:
          "Указ Президента №214/2026 ввів у дію блокуючі санкції проти ТОВ 'СпецТехПостач' строком на 5 років.",
        targetEntityId: "comp-1",
        entityName: "ТОВ 'СпецТехПостач'",
      ...
      },
      {
        ...
        description:
          "Коваленку І.В. оголошено підозру за ст. 110-2 ККУ (фінансування дій, вчинених з метою зміни меж території України).",
  ...
  const displayedEntitiesInfo = useMemo(() => {
    return [
      ...
      {
        id: "wallet-1",
        key: "wallet1" as const,
        name: "BTC Wallet (0x38ac...)",
        currentRisk: 89,
        color: "#c084fc",
        hoverColor: "#a855f7",
        type: "Криптогаманець",
      ...
      {
        id: "person-1",
        key: "person1" as const,
        name: "Коваленко Ігор В.",
        currentRisk: 82,
        color: "#f59e0b",
        hoverColor: "#fbbf24",
        type: "Фізособа",
  ...
  return (
    <div
      className="bg-slate-900/40 border border-slate-800 rounded-2xl shadow-[0_4px_40px_rgba(30,58,138,0.15)] backdrop-blur-md p-2 relative overflow-hidden flex flex-col space-y-4"
      id="d3-historical-trends-widget"
    >
          ...
          >
            ...
            >
              {[100, 80, 60, 40, 20, 0].map((val) => (
                <div key={val} className="w-full flex items-center">
                  <span className="w-10 text-xs font-mono text-slate-600 text-right pr-2">{val}%</span>
            ...
            >
              ...
              {visibleEntities.comp1 && activeTab !== "suspicious" && (
                <path
                  d={areaGenerator("comp1")}
                  fill="url(#gradient-comp1)"
                  className="transition-all duration-300 pointer-events-none"
                />
              )}
              {visibleEntities.wallet1 && (
                <path
                  d={areaGenerator("wallet1")}
                  fill="url(#gradient-wallet1)"
                  className="transition-all duration-300 pointer-events-none"
                />
              )}
              {visibleEntities.person1 && (
                <path
                  d={areaGenerator("person1")}
                  fill="url(#gradient-person1)"
                  className="transition-all duration-300 pointer-events-none"
                />
              )}
              {visibleEntities.comp2 && activeTab === "all" && (
                <path
                  d={areaGenerator("comp2")}
                  fill="url(#gradient-comp2)"
                  className="transition-all duration-300 pointer-events-none"
                />
              )}
            ...
            <div
              className="absolute bottom-0 inset-x-0 flex justify-between px-3 select-none text-xs font-mono text-slate-500"
              style={{ paddingLeft: padding.left + 5, paddingRight: padding.right + 5, height: padding.bottom }}
            >
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1 font-mono">
                      ...
                      <span
                        className={`px-2 py-1 rounded text-[7.5px] font-bold ${
