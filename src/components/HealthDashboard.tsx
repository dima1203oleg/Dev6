...
export default function HealthDashboard() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "LIVE": return "text-green-500 bg-green-500/10 border-green-500/50";
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm font-mono uppercase tracking-widest">Real-time Connector Status Dashboard</p>
          </div>
        </div>
        <button 
          onClick={fetchHealth}
          ...
          health.map((service) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase ${getStatusColor(service.status)}`}>
                  {service.status}
                </span>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800">
              {[
                { type: "SEARCH", user: "kizimalesik@gmail.com", action: "PREDATOR_CORE_SEARCH", time: "2 min ago", status: "SUCCESS" },
                { type: "VIEW", user: "kizimalesik@gmail.com", action: "INTELLIGENCE_DOSSIER", time: "5 min ago", status: "SUCCESS" },
                { type: "AUTH", user: "system_orchestrator", action: "API_CONNECTOR_AUTH", time: "10 min ago", status: "SUCCESS" },
              ].map((log, i) => (
