...
import { OsintEntity, OSINT_ENTITIES } from "../types/osint";
  ...
  const connectedEntities = React.useMemo(() => {
    ...
    if (riskFilter !== "ALL") {
      result = result.filter((conn) => {
        ...
        if (riskFilter === "HIGH")
          return (conn.entity.riskScore >= 50 && conn.entity.riskScore < 75) || conn.risk === "MEDIUM";
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-12 px-3 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 max-w-xs mt-1.5 leading-relaxed font-sans">
                    Клікніть на будь-яку компанію, засновника, відкриту бібліотеку або вузол графа залежностей для
                    миттєвого інспектування метаданих.
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-6 text-xs">
                  ...
                  <div className="space-y-5" id="inspector-osint-context">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        ...
                        <span
                          className={`text-xs font-mono font-bold px-2 py-1 rounded border ${selectedEntity.riskScore > 75 ? "text-red-400 border-red-500/20 bg-red-500/5" : "text-slate-300 border-slate-800"}`}
                        >
                      ...
                      <button
                        onClick={() => {
                          const dataStr =
                            ...
                            encodeURIComponent(JSON.stringify(selectedEntity, null, 2));
                          const downloadAnchorNode = document.createElement("a");
                          downloadAnchorNode.setAttribute("href", dataStr);
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-blue-400 border border-slate-800 font-bold py-2 px-3 rounded-lg text-xs transition-colors cursor-pointer"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="bg-black/30 border border-slate-800 rounded-lg p-2 space-y-1.5 font-mono text-xs text-slate-300">
                        <div className="flex justify-between">
                          <span className="text-slate-500">Адреса:</span>
                          <span
                            className="text-slate-200 text-right max-w-[160px] truncate"
                            title={selectedEntity.address}
                          >
                    ...
                    <div className="space-y-2">
                      <span className="text-xs text-slate-500 font-mono font-bold uppercase tracking-widest block">
                        Пов'язані об'єкти з БД OSINT (Connected Entities)
                      </span>
                      <div className="space-y-2">
                        {connectedEntities.length === 0 ? (
                          <div className="text-xs text-slate-500 font-mono p-2 bg-slate-900/20 rounded-lg border border-slate-800/60">
                            Не знайдено пов'язаних об'єктів у базі
                          </div>
                        ) : (
                          connectedEntities.map((conn, idx) => (
                            <div
                              key={idx}
                              className="bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border border-slate-800 p-2 rounded-lg flex items-center justify-between text-xs hover:border-slate-800 hover:shadow-[0_0_15px_rgba(59,130,246,0.15)] transition-all duration-300 transition-all cursor-default"
                            >
                              <div className="flex items-center gap-2">
                                <div
                                  className={`w-8 h-8 rounded-full flex items-center justify-center ${conn.entity.type === "company" ? "bg-blue-500/10 text-blue-400" : conn.entity.type === "person" ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}
                                >
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-1.5 mt-1">
                                    <span className="text-xs text-blue-400 font-mono uppercase bg-blue-500/10 px-1 rounded">
                                      {conn.direction === "outgoing" ? "Вихідний" : "Вхідний"}
                                    </span>
                              ...
                              <div className="flex flex-col items-end gap-1">
                                <span
                                  className={`text-xs font-bold px-2 py-1 rounded border ${conn.risk === "HIGH" ? "text-red-400 bg-red-500/5 border-red-500/15" : "text-slate-300 bg-slate-900/50 border-slate-800"}`}
                                >
                                  {conn.risk}
                                </span>
                    ...
                    <div className="bg-indigo-950/15 border border-indigo-900/30 rounded-lg p-2 space-y-2">
                      <div className="flex items-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                        <span className="text-xs text-blue-400 font-mono font-bold uppercase tracking-widest">
                          ШІ-РЕКОМЕНДАЦІЇ NEXUS
                        </span>
                      </div>
                    ...
                    {selectedEntity.rawContext && (
                      <div className="space-y-2 pt-2 border-t border-slate-800">
                        ...
                        <div className="bg-slate-950/50 border border-slate-800 rounded-lg p-2 space-y-3 text-xs text-slate-300 font-mono overflow-hidden">
                            ...
                            <div>
                              ...
                              <ul className="list-disc pl-3 space-y-1">
                                {selectedEntity.rawContext.nacp.map((d: any, idx: number) => (
                                  <li key={idx} className="truncate">
                                    {d.first_name} {d.last_name} ({d.work_place})
                                  </li>
                                ))}
                              </ul>
                            </div>
                ...
                {selectedTool && (
                  <div className="space-y-5" id="inspector-tool-context">
                    ...
                    <div className="bg-black/30 border border-slate-800 p-2 rounded-lg space-y-2">
                      <span className="text-xs text-slate-500 font-mono font-bold uppercase tracking-widest block">
                        Сумісність з ТЗ
                      </span>
                      <div className="flex items-center gap-2">
                        <span className="text-2xl font-bold font-mono text-blue-400">
                          {selectedTool.compatibilityScore}%
                        </span>
                        <div className="flex-1">
                          <div className="w-full bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] rounded-full h-1.5">
                            <div
                              ...
                              style={{ width: `${selectedTool.compatibilityScore}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5">
                        <span className="text-xs text-red-400 font-mono font-bold uppercase tracking-widest block">
                          Недоліки / Ризики ліцензування
                        </span>
                        <ul className="space-y-1 text-slate-300 list-disc list-inside pl-1 text-xs">
                          {selectedTool.disadvantages.map((dis, i) => (
                            <li key={i} className="leading-relaxed">
                              {dis}
                            </li>
                          ))}
                    ...
                    <div className="space-y-1.5">
                      <span className="text-xs text-slate-500 font-mono font-bold uppercase tracking-widest block">
                        Опис архітектурної ролі
                      </span>
                      <p className="text-slate-300 leading-relaxed text-xs p-2 rounded-lg bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border border-slate-800 font-sans">
                        {selectedTool.role}
                      </p>
                    </div>
                  </div>
                ...
                {selectedNode && (
                  <div className="space-y-5" id="inspector-node-context">
                    ...
                    {/* Status metrics */}
                    <div className="bg-black/30 border border-slate-800 p-2 rounded-lg space-y-2 font-mono text-xs">
                    ...
                    <div className="space-y-1.5">
                      <span className="text-xs text-slate-500 font-mono font-bold uppercase tracking-widest block">
                        Призначення за архітектурою
                      </span>
                      <p className="text-slate-300 leading-relaxed text-xs p-2 rounded-lg bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border border-slate-800">
                        {selectedNode.details ||
                          "Цей контейнер забезпечує автоматизоване завантаження, первинну фільтрацію та індексування великих масивів даних судових ухвал та митних декларацій."}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5 font-mono">
                      ...
                      <div className="bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border border-slate-800 p-2 rounded-lg text-xs text-slate-300 space-y-1">
                        <p>
                          PORT: <span className="text-slate-200">3000 / internal TCP</span>
                        </p>
                        <p>
                          CONTAINER: <span className="text-slate-200">Docker CloudRun Node v18</span>
                        </p>
                        <p>
                          LOGS: <span className="text-slate-500">Listening on 0.0.0.0:3000... success</span>
                        </p>
                      </div>
                    </div>
                  </div>
