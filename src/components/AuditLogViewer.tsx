...
export default function AuditLogViewer() {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      case 'LOGIN': return <CheckCircle className="w-4 h-4 text-emerald-400" />;
      case 'SEARCH': return <Search className="w-4 h-4 text-blue-400" />;
      case 'AI_REQUEST': return <Database className="w-4 h-4 text-purple-400" />;
      case 'EXPORT': return <Database className="w-4 h-4 text-amber-400" />;
      case 'REPORT': return <Eye className="w-4 h-4 text-indigo-400" />;
      case 'SYSTEM': return <ShieldAlert className="w-4 h-4 text-rose-400" />;
  ...
  const getActionColor = (action: string) => {
    switch (action) {
      ...
      case 'SEARCH': return 'text-blue-400 bg-blue-400/10 border-blue-400/20';
      case 'AI_REQUEST': return 'text-purple-400 bg-purple-400/10 border-purple-400/20';
      case 'EXPORT': return 'text-amber-400 bg-amber-400/10 border-amber-400/20';
      case 'REPORT': return 'text-indigo-400 bg-indigo-400/10 border-indigo-400/20';
      case 'SYSTEM': return 'text-rose-400 bg-rose-400/10 border-rose-400/20';
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/50 text-sm">
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-slate-500">Завантаження журналу...</td>
                </tr>
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-900/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[11px] font-bold tracking-wider ${getActionColor(log.action)}`}>
                    ...
                    <td className="px-6 py-4 text-slate-400 w-full truncate">
                      {log.details}
                    </td>
                  </tr>
                ))
