...
const PROFILER_PEOPLE: ProfilerPerson[] = [
  ...
  },
  {
    ...
    role: 'ФОП / Фізична особа (ІПН 3111724753)',
    age: 41,
    dob: '12.03.1985',
    passport: 'КЕ 402198',
    taxId: '3111724753',
    ...
    isNomineeProxy: false,
    psychoProfile: {
      riskTolerance: 'Законна прозора діяльність (ФОП). Нульова толерантність до тіньових схем, судових спорів чи корупційних афіліацій.',
      travelPattern: 'Внутрішні переміщення по території України, офіційні ділові поїздки.',
      spendingHabits: 'Раціональне споживання, відсутність аномальних витрат або непідтвердженого розкошу.',
...
const PROFILER_ASSETS: ProfilerAsset[] = [
  // Assets for Kizyma Dmytro (Verified FOP, registered IPN 3111724753)
  {
    id: 'asset-kizyma-fop',
    type: 'business',
    name: 'ФОП Кізима Дмитро Миколайович (ІПН 3111724753)',
    value: 'Діючий ФОП (100%)',
    valueNum: 150000,
    registeredToId: 'kizyma-dmytro',
    registeredToName: 'Кізима Дмитро Миколайович',
    relationType: 'Пряма власність (100%)',
    isNominee: false,
...
export default function PersonProfiler() {
  ...
  return (
    <div className="flex-1 p-2 sm:p-3 md:p-4 flex flex-col gap-3 sm:gap-4 bg-slate-950 h-full w-full" id="person-profiler-root">
      ...
      <AnimatePresence>
        {isGeneratingDossier && (
          ...
          >
            <div className="bg-slate-900 border border border-slate-800 p-4 rounded-lg w-full w-full shadow-2xl space-y-6 relative overflow-hidden">
      ...
      <div className="bg-black/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] border border-slate-800 p-2 rounded-lg">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2">
          ...
          <button
            ...
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/40 border border-slate-800 text-xs font-bold text-indigo-300 hover:text-white font-mono uppercase tracking-wider cursor-pointer transition-all shrink-0"
          >
          ...
          <button
            ...
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-600/20 hover:bg-rose-600 border border-slate-800 text-xs font-bold text-rose-400 hover:text-white font-mono uppercase tracking-wider cursor-pointer transition-all shrink-0 shadow-lg shadow-rose-950/20"
          >
          ...
          <button
            type="button"
            onClick={handlePurgeAllNoise}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-600/30 hover:bg-rose-600 border border-rose-500/50 text-xs font-bold text-rose-200 hover:text-white font-mono uppercase tracking-wider cursor-pointer transition-all shrink-0 shadow-lg shadow-rose-950/40"
            title="Повністю очистити результати пошуку, видалити сторонні компанії та шум"
          >
          ...
          <form onSubmit={handleCustomSearch} className="flex items-center gap-2 bg-slate-900/60 p-1 rounded-lg border border-slate-800 w-full w-full">
            <Search className="w-3.5 h-3.5 text-slate-500 ml-2.5 shrink-0" />
            <input
              type="text"
              placeholder="ШІ-пошук особи (напр. 'Кізима' або ІПН)..."
            ...
            {customSearchName && (
              <button
                type="button"
                onClick={handlePurgeAllNoise}
                className="px-2 py-1 bg-rose-500/20 hover:bg-rose-500 text-rose-300 hover:text-white rounded-xl text-xs font-bold font-mono transition-all shrink-0 flex items-center gap-1"
                title="Очистити поле та стерти всі сторонні записи"
              >
                <X className="w-3 h-3" />
                <span>Очистити</span>
              </button>
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:opacity-50 text-xs font-bold text-white font-mono uppercase tracking-wider cursor-pointer transition-all shrink-0"
            >
        ...
        {isPurgedNoise && (
          <div className="mt-3 p-2.5 bg-emerald-950/40 border border-emerald-500/30 rounded-xl flex items-center justify-between text-xs font-mono text-emerald-300">
            <div className="flex items-center gap-2">
              <Check className="w-4 h-4 text-emerald-400 shrink-0" />
              <span>
                <strong>ШІ-Очищення шуму активовано:</strong> Сторонні компанії та родинний масив (Богдан Іванович, 29 записів YouControl) повністю відокремлені та видалені.
              </span>
            </div>
            <button
              onClick={() => setIsPurgedNoise(false)}
              className="text-xs text-slate-400 hover:text-white underline ml-2 shrink-0 cursor-pointer"
            >
              Скинути фільтр
            </button>
          </div>
          ...
          {PROFILER_PEOPLE.map((p) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col text-left p-2.5 rounded-lg border transition-all duration-300 relative overflow-hidden cursor-pointer ${
                  isSelected 
      ...
      <AnimatePresence>
        {isAiSearching && (
          ...
          >
            <div className="bg-slate-950/80 backdrop-blur-xl border border-slate-800 rounded-lg p-2 shadow-2xl font-mono text-xs space-y-1 bg-gradient-to-br from-slate-950 to-slate-900">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-2">
        ...
        <div className="xl:col-span-7 space-y-6">
          ...
          <div className="flex items-center gap-1.5 border-b border-slate-800/60 pb-px">
            {(['profile', 'assets', 'nominees', 'audit', 'behavioral', 'adverse'] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-2 py-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-wider border-b-2 transition-all cursor-pointer relative ${
                  activeTab === tab 
              ...
              >
                ...
                {tab === 'audit' && 'ШІ-Аудит'}
                {tab === 'behavioral' && 'Behavioral Intelligence'}
                {tab === 'adverse' && (
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'profile' && (
              ...
              >
                {/* Profile Card & Info */}
                <div className="bg-slate-900/40 border border-slate-800/60 rounded-lg p-2 relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-2 justify-between border-b border-slate-800/60 pb-4 mb-4">
                    <div className="flex items-start gap-2.5">
                      <div className="p-2 bg-slate-950/80 border border-slate-800 rounded-lg text-blue-400">
                    ...
                    <div className="flex items-center gap-2">
                      {selectedPerson.isNomineeProxy && (
                        <span className="px-2.5 py-1 bg-amber-500/10 border border-slate-800 text-amber-500 text-xs font-bold uppercase tracking-wider rounded-lg flex items-center gap-1">
                          <AlertTriangle className="w-3.5 h-3.5" />
                          <span>НОМІНАЛ / ПРОКСІ</span>
                        </span>
                      )}
                      <span className={`px-2.5 py-1 rounded-lg border text-xs font-mono font-bold uppercase tracking-wider ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <div className="bg-slate-900/40 border border-slate-800/60 rounded-lg p-2 space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-purple-500/20 text-purple-300 border border-purple-500/40 hover:bg-purple-500/30 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer w-fit"
                    >
                      <Building className="w-3.5 h-3.5 text-purple-400" />
                      <span>+ Уточнити підприємство (Співзасновник / ТОВ)</span>
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {allAssetsForView.map((asset) => {
                      ...
                      return (
                        <div 
                          key={asset.id} 
                          className={`bg-slate-900/40 border rounded-lg p-2 flex flex-col md:flex-row md:items-center justify-between gap-2 transition-all hover:bg-black/40 transition-colors ${
            ...
            {activeTab === 'nominees' && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3 }}
                className="space-y-5 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2"
              >
                <div className="flex items-center justify-between border-b border-slate-800/60 pb-3">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {PROFILER_PEOPLE.filter(p => p.id !== 'kovalenko-ihor').map((rel) => {
                    ...
                    return (
                      <div key={rel.id} className="bg-black/40 border border-slate-800 rounded-lg p-2 space-y-3.5">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
                          <div className="flex items-center gap-2.5">
                            <div className="p-1.5 bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-lg text-slate-400">
                        ...
                        <div className="space-y-1.5 pt-1">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
                            {assetsRegistered.map((a) => (
                              <div key={a.id} className="bg-black/30 border border-slate-800/60 p-2 rounded-lg flex items-center justify-between gap-1">
                        ...
                        <div className="bg-rose-500/5 border border-slate-800 rounded-lg p-2.5 flex items-start gap-2.5 text-xs leading-relaxed">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/60 pb-3">
                  ...
                  <button
                    onClick={runAudit}
                    disabled={isAuditing}
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:opacity-50 text-xs font-mono font-bold uppercase text-white cursor-pointer transition-all active:scale-95 shadow-2xl shadow-black/40 shadow-blue-950/50 shrink-0"
                  >
                ...
                {/* Audit Terminal Log */}
                <div className="bg-slate-950/80 border border-slate-800 rounded-lg p-2 font-mono text-xs h-[260px] overflow-y-auto custom-scrollbar flex flex-col gap-1.5 bg-gradient-to-b from-slate-950 to-slate-900 relative">
                  {auditLog.length === 0 ? (
                ...
                {auditLog.length > 0 && !isAuditing && (
                  <div className="bg-black/40 border border-slate-800 rounded-lg p-2 flex items-center justify-between text-xs font-mono">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600/20 hover:bg-rose-600 text-rose-400 hover:text-white border border-slate-800 rounded-lg text-xs font-bold uppercase transition-all duration-300"
                    >
        ...
        <div className="xl:col-span-5 space-y-6">
          ...
          <div className="bg-black/40 backdrop-blur-md border border-slate-800/60 rounded-lg p-2 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800/60 pb-3">
              <div className="flex items-center gap-2">
                  ...
                  selectedPerson.name.toLowerCase().includes('кізима') || selectedPerson.name.toLowerCase().includes('кизима') ? (
                    <>
                      ...
                      <g stroke="#334155" strokeWidth="1.5">
                        ...
                        <line x1="250" y1="180" x2="110" y2="270" stroke="#f43f5e" strokeDasharray="4 4" />
                        <line x1="250" y1="180" x2="390" y2="180" stroke="#3b82f6" strokeDasharray="4 4" />
                      </g>
                      ...
                      <g fontSize="7" fontWeight="bold" fontFamily="monospace" textAnchor="middle">
                        ...
                        <text x="180" y="123" fill="#fbbf24">BMW X5 (7 штрафів)</text>
                        
                        <rect x="135" y="225" width="90" height="12" rx="4" fill="#0f172a" stroke="#f43f5e" strokeOpacity="0.4" />
                        <text x="180" y="233" fill="#f87171">VW Crafter (Шлях)</text>
                        
                        <rect x="275" y="165" width="100" height="12" rx="4" fill="#0f172a" stroke="#3b82f6" strokeOpacity="0.4" />
                        <text x="325" y="173" fill="#60a5fa">ТОВ Угерсько-Транс</text>
                        
                        <rect x="275" y="185" width="100" height="12" rx="4" fill="#0f172a" stroke="#3b82f6" strokeOpacity="0.4" />
                        <text x="325" y="193" fill="#60a5fa">Закриті підприємства (8+)</text>
                      </g>
                      ...
                      <g className="cursor-pointer">
                        <circle cx="110" cy="90" r="18" fill="#020617" stroke="#f59e0b" strokeWidth="2" />
                        <text x="110" y="93" textAnchor="middle" fill="#fbbf24" fontSize="7" fontWeight="bold" fontFamily="monospace">РОДИЧ</text>
                        <text x="110" y="120" textAnchor="middle" fill="#94a3b8" fontSize="8" fontWeight="bold">Кізима Н.М.</text>
                      </g>
                      
                      <g className="cursor-pointer">
                        <circle cx="110" cy="270" r="18" fill="#020617" stroke="#f43f5e" strokeWidth="2" />
                        <text x="110" y="273" textAnchor="middle" fill="#f87171" fontSize="7" fontWeight="bold" fontFamily="monospace">ПАРТНЕР</text>
                        <text x="110" y="300" textAnchor="middle" fill="#94a3b8" fontSize="8" fontWeight="bold">Бізнес-партнер</text>
                      </g>
                    </>
                  ) : (
                    <>
      ...
      {/* Modal for adding/verifying co-founded companies */}
      <AnimatePresence>
        {showAddCompanyModal && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="bg-slate-900 border border-slate-800 rounded-lg p-6 w-full w-full shadow-2xl space-y-4"
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center gap-2">
                  <Building className="w-5 h-5 text-purple-400" />
                  <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider">
                    Вказати фірму (Співзасновник / ТОВ)
                  </h3>
                </div>
                <button
              ...
              <form onSubmit={handleAddCompany} className="space-y-4 text-xs font-mono">
                <div>
                  <label className="block text-slate-400 mb-1">Назва підприємства / ТОВ *</label>
                  <input
                    type="text"
                    required
                    placeholder='напр., ТОВ "Агро-Технолоджи" або "Альфа Інвест"'
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-slate-400 mb-1">Код ЄДРПОУ (опціонально)</label>
                    <input
                      type="text"
                      placeholder="8-значний код"
                      value={newCompanyEdrpou}
                      onChange={(e) => setNewCompanyEdrpou(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-purple-500"
                    />
                  </div>
                  <div>
                    <label className="block text-slate-400 mb-1">Розмір частки (%)</label>
                    <input
                      type="text"
                      placeholder="50%"
                      value={newCompanyShare}
                      onChange={(e) => setNewCompanyShare(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-purple-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-slate-400 mb-1">Роль / Статус в ЄДР</label>
                  <select
                    value={newCompanyRole}
                    onChange={(e) => setNewCompanyRole(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-purple-500"
                  >
                    <option value="Співзасновник (Частка)">Співзасновник (Частка статутного капіталу)</option>
                    <option value="Бенефіціарний власник (КБВ)">Кінцевий бенефіціарний власник (КБВ)</option>
                    <option value="Керівник / Директор">Керівник / Директор</option>
                  </select>
                </div>

                <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-xl text-purple-300 text-[11px] leading-relaxed">
                  💡 Прив'язка за точним кодом ЄДРПОУ або назвою усуває плутанину з однофамільцями, родичами чи застарілими записами в YouControl / Opendatabot.
                </div>
