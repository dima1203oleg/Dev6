...
export function LiveChatBot() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = async (event) => {
      ...
      if (msg.audio && outputAudioCtxRef.current) {
        ...
        (window as any).__isLiveWebAudioPlaying = true;
        ...
        // CRITICAL FIX: Account for playbackRate in duration calculation so chunks don't overlap!
    ...
    ws.onerror = (e) => {
      console.warn("[LiveChatBot WS error]", e);
    };
  ...
  useEffect(() => {
    const handleStopMicEvent = () => {
      stopMic();
    };
    window.addEventListener('stop-live-chatbot-mic', handleStopMicEvent);
    return () => {
      window.removeEventListener('stop-live-chatbot-mic', handleStopMicEvent);
    };
  }, []);

  const startMic = async () => {
    try {
      window.dispatchEvent(new CustomEvent('stop-voice-call'));
      if (typeof window !== "undefined" && "speechSynthesis" in window) {
        try { window.speechSynthesis.cancel(); } catch (_) {}
      }
      setError(null);
      ...
      console.warn("LiveChatBot Mic Error:", err?.message || err);
      let errMsg = "Не вдалося запустити мікрофон.";
      if (err?.name === 'NotAllowedError' || err?.name === 'PermissionDeniedError' || err?.name === 'SecurityError' || String(err?.message || '').includes('not allowed')) {
        errMsg = "Доступ до мікрофона відхилено або обмежено браузером/iFrame.";
      } else if (err?.message) {
        errMsg = err.message;
      }
      setError(errMsg);
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-4">
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-lg p-2 text-xs leading-relaxed ${msg.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-black/40 border border-slate-800 text-slate-300'}`}>
            ...
            <div className="p-2 bg-slate-900/60 border-t border-slate-800">
              <form onSubmit={handleSendText} className="flex items-center gap-1.5 bg-slate-950/40 backdrop-blur-md shadow-[0_4px_40px_rgba(30,58,138,0.15)] p-1.5 rounded-lg border border-slate-800/80">
                <button
                  type="button"
