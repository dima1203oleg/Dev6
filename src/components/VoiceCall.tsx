...
export function VoiceCall() {
  ...
  useEffect(() => {
    const handleStopCallEvent = () => {
      stopCall();
    };
    window.addEventListener('stop-voice-call', handleStopCallEvent);
    return () => {
      window.removeEventListener('stop-voice-call', handleStopCallEvent);
    };
  }, []);

  const startCall = async () => {
    try {
      window.dispatchEvent(new CustomEvent('stop-live-chatbot-mic'));
      ...
      console.warn("VoiceCall Mic Error:", err?.message || err);
