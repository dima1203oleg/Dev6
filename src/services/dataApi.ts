...
export const dataApi = {
  ...
  fxSeries: (code: string, days: number) => request<FxAnalytics>(`/fx/series?code=${encodeURIComponent(code)}&days=${days}`),
  crypto: () => request<CryptoSpot>("/crypto"),
  openDataOverview: () => request<OpenDataOverview>("/opendata/overview"),
  openDataSearch: (query: string, rows = 20) => request<OpenDataSearchData>(`/opendata/search?q=${encodeURIComponent(query)}&rows=${rows}`),
  procurementSearch: (query: string, rows = 20) => request<ProcurementAnalytics>(`/procurement/search?q=${encodeURIComponent(query)}&rows=${rows}`),
