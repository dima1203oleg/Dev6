...
export function convertEntityToStix21(entity: OsintEntity): StixBundle {
  ...
  const stixObjectType =
    entity.type === "company" ? "identity" : entity.type === "person" ? "identity" : "infrastructure";

  const mainStixObject: Record<string, any> = {
    type: stixObjectType,
    spec_version: "2.1",
    id: `${stixObjectType}--${crypto.randomUUID()}`,
    created: timestamp,
    modified: timestamp,
    name: entity.name,
  ...
  if (entity.relations && entity.relations.length > 0) {
    entity.relations.forEach((rel) => {
      const relTargetId = `identity--${crypto.randomUUID()}`;
